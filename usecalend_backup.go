...
func DrawMarkWater(dc *gg.Context){
    ...
    str="Copyright 2019 by Horacio Useche Losada, usesoft33@gmail.com"
    iSize=10
    y=fHEIPAPER-float64(iSize)
    _=DrawTextAlign(dc,y,str,63,iSize,"right",color.RGBA{0,0,0,245})
    str="Imágenes de Caño Cristales, Meta"
...
func DrawCredits(dc *gg.Context){
	...
	y+=float64(iSize)
    str="Crypto: software to encrypt information, text, videos, images, audios, etc. All your files can be "
    str+="protected with Crypto"
    DrawText(dc,x,y,str,iFont,iSize,clrText)
    y+=float64(iSize)
...
func DrawYearImg3x4(dc *gg.Context,iYear int){
    ...
    sYear:=fmt.Sprintf("%d",iYear)
    h:=DrawTextAlign(dc,vaY,sYear,64,20,"center",color.RGBA{0,0,0,255})
    vaY+=h
    str:="Merry Christmas and Happy New Year"
    //str:="Feliz Navidad y Próspero Año"
    DrawTextAlign(dc,vaY,str,62,20,"center",color.RGBA{128,128,128,255})
    vaY+=h
    str="Are the wishes of SCAM and ORIGIN App"
    //str="Les desea familia Pueto Useche"
    h=DrawTextAlign(dc,vaY,str,63,10,"center",color.RGBA{0,0,0,255})
    vaY+=h
    ...
    h=DrawTextAlign(dc,vaY,str,63,8,"center",color.RGBA{0,0,0,255})
    vaY+=2*h
    vaX=fMARGIN_IZQ
    iMes:=0
    ind:=0
    ...
    vaY+=GetHeiMes()
	str="Copyright 2019 Horacio Useche"
	DrawCodeBarClass(dc,str,15,510,1,10,6)
    DrawTextFestivos(dc,fMARGIN_IZQ,vaY,iYear)
    DrawMarkWater(dc)
    DrawCredits(dc)
}
...
func DrawYearImg1x1(dc *gg.Context,iYear int){
    ...
    // organizando 4 filas
    var vaX,vaY float64
    vaY=fMARGIN_TOP+90.0
    sYear:=fmt.Sprintf("%d",iYear)
    h:=DrawTextAlign(dc,vaY,sYear,64,90,"center",color.RGBA{0,0,0,255})
    vaY+=h
    str:="Merry Christmas and Happy New Year"
    DrawTextAlign(dc,vaY,str,62,90,"center",color.RGBA{128,128,128,255})
    vaY+=h
    str="Are the wishes of ORIGIN"
    h=DrawTextAlign(dc,vaY,str,63,40,"center",color.RGBA{0,0,0,255})
    vaY+=h
    str="The software banned by google because everything good is dangerous"
    h=DrawTextAlign(dc,vaY,str,63,28,"center",color.RGBA{0,0,0,255})
    vaX=fMARGIN_IZQ
    vaY+=h
    // dibuja la imagen
...
func main(){   
 ...
 sPathImg="/home/xun33/dev/devgo/projects/calend/imgs/cristales/img_"
 sPathFont="/home/xun33/dev/Fonts/Serial/font_"
 sPath1x1="/home/xun33/dev/devgo/projects/calend/imgs/fractal_2/gfg.png"
 SetTypeCalendar(true)
 SetDrawFestivo(false)
 BuildYear(2025)
